...
void setDrivingMode(bool sportMode)
{
  ...
  // Update PWM immediately if any vertical motors are active
  if (motorForward || motorBackward)
  {
    analogWrite(ENA_PIN, enaSpeed);
  }
  
  // Also add debug output to verify the mode change
  Serial.print("Driving mode changed to: ");
  Serial.println(sportMode ? "SPORT" : "NORMAL");
  Serial.print("New ENA speed: ");
  Serial.println(enaSpeed);
}
...
void handleWebSocketMessage(AsyncWebSocketClient *client, String message)
{
  ...
  // Handle ping commands for latency measurement
  if (command.startsWith("ping:"))
  {
    String pingId = command.substring(5);
    String pongResponse = "pong:" + pingId;
    client->text(pongResponse);
    Serial.println("Ping received, sending pong: " + pongResponse);
    return;
  }
